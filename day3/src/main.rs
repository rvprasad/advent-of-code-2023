...
fn main() {
    ...
    if let Ok(content) = get_content(&args[1]) {
        let symbol_and_affected_positions = get_symbol_affected_positions(&content);
        let part_num_and_positions = get_part_num_to_positions(&content);
...
fn get_content(filename: &String) -> std::io::Result<Vec<Vec<char>>> {
    ...
    let lines = reader.lines().flatten().filter_map(|l| {
        if l.is_empty() {
            None
        } else {
...
fn get_symbol_affected_positions(content: &Vec<Vec<char>>) -> Vec<(char, HashSet<(i32, i32)>)> {
    let mut ret = Vec::new();
    ...
    for (row_num, row) in content.iter().enumerate() {
        for (col_num, elem) in row.iter().enumerate() {
            ...
            let row_i32 = row_num as i32;
            let col_i32 = col_num as i32;
            for r in -1..=1 {
                for c in -1..=1 {
...
fn get_part_num_to_positions(content: &Vec<Vec<char>>) -> Vec<(i32, HashSet<(i32, i32)>)> {
    let mut ret = Vec::new();
    ...
    for (row_num, row) in content.iter().enumerate() {
        ...
        for (col_num, elem) in row.iter().enumerate() {
            let mut col_i32 = col_num as i32;
            ...
            if process_captured_digits {
                let num_positions = HashSet::from_iter(
                    (0..(captured_digits.len() as i32)).map(|i| (row_i32, col_i32 - i)),
                );
                if let Ok(number) =
                    i32::from_str_radix(&String::from_iter(captured_digits.clone()), 10)
                {
